...
| `llm.api_key` | API Key | — |
| `llm.model` | 模型名称 | — |
| `llm.name` | 数字人名字，影响唤醒词和系统提示词 | `小冉` |
| `wake_word` | 自定义唤醒词，留空则自动生成 | 空（自动） |
| `enable_fbx` | 是否加载 FBX 动画（空闲时会随机播放动作） | `true` |

### 唤醒词

如果不配置 `wake_word`，程序会自动从 `llm.name` 生成。例如名字为"小冉"时，唤醒词为"小冉小冉"（名称重复两遍）。
...
| 方式 | 操作 |
|------|------|
| **语音唤醒** | 说出唤醒词（默认"小冉小冉"），数字人回应"哎，我在呢"后即可说话 |
| **点击/按键** | 点击数字人角色，或按**空格键** / **回车键**，数字人进入聆听状态 |

### 对话流程

1. **唤醒**：说"小冉小冉"或点击/按键 → 数字人回应"哎，我在呢"
...
- 确保麦克风工作正常，没有被其他程序占用
- 尝试用鼠标点击或按空格键手动触发对话
- 如果 `name` 不是"小冉"，唤醒词会自动变化，确认当前唤醒词
