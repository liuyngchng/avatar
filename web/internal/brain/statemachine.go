...
type StateMachine struct {
	...
	// mic audio is dropped. Set by sayGreeting or tap handler to prevent
	// TTS echo from reaching the ASR/VAD.
	suppressUntil int64
}
	...
	case "tap", "wake_detected":
		...
		go func() {
			if ev.Type == "wake_detected" {
				...
				// Tap-to-talk: suppress ~200ms of mic audio so the
				// previous turn's TTS tail doesn't reach ASR.
				sm.mu.Lock()
...
func (sm *StateMachine) pipeline() {
	...
	for turn := 1; ; turn++ {
		...
		// ── Phase 2+3: THINKING → SPEAKING (streamed, sentence-by-sentence) ──
...
func (sm *StateMachine) collectSpeech() []float32 {
	...
	for {
		...
		case samples, ok := <-asrBuf:
			...
			// echo never reaches the ASR or VAD.
			sm.mu.Lock()
...
// streamReply streams the LLM response and synthesizes + sends audio
// sentence by sentence as soon as each complete sentence arrives (pipelined),
// matching Android's producer/consumer pipeline. Returns the full reply text,
// the detected emotion, and whether at least one sentence was spoken.
func (sm *StateMachine) streamReply(userText string) (fullReply string, emotion Emotion, spoke bool) {
	...
	// synthesize each sentence while the previous one is being sent to the browser.
	go func() {
		...
		for {
			...
			case chunk, ok := <-chunkCh:
					...
					if last != "" {
						...
						if normalized != "" {
							...
							if err == nil && len(result.Samples) > 0 {
								...
								speechCh <- SpeechChunk{Samples: result.Samples, Text: last, SampleRate: result.SampleRate}
							}
						}
					}
					return
				...
				for _, sentence := range complete {
					...
					speechCh <- SpeechChunk{Samples: result.Samples, Text: sentence, SampleRate: result.SampleRate}
				}
			case err := <-errCh:
				if err != nil {
	...
	// Consumer: send each synthesized sentence to the browser for playback.
	for chunk := range speechCh {
		...
		sm.sendAudioOut(&tts.SynthesizeResult{
...
// speakFallback synthesizes a hardcoded fallback reply and sends it to the
