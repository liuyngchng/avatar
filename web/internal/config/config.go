...
// NoSpeechTimeout returns how long the multi-turn conversation waits for
// the user to start speaking after wake-up or after the previous turn
// ends before automatically closing the dialogue (so the user must re-wake
// to talk again). Defaults to 5 seconds when the config key is not set
// or is non-positive.
